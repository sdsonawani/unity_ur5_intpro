...
{
    [Serializable]
    public class NatNetException : Exception
    {
    ...
    internal static class NatNetLogging
    {
        ...
        private static NatNetLogEventArgs m_nativeLogEventArgs = new();
        ...
        static NatNetLogging()
        {
            ...
            NativeMethods.NatNet_SetLogCallback(m_nativeLogHandler);
        }
...
#pragma warning restore CA1031 // Do not catch general exception types
            {
                ...
                System.Diagnostics.Debug.WriteLine("ERROR - Exception occurred in LogCallbackNativeThunk: " +
                                                   ex.ToString());
            }
        }
    }
    ...
    internal class NatNetServerDiscovery : IDisposable
    {
        ...
        private NatNetServerDiscoveredEventArgs m_serverDiscoveredEventArgs = new();
        private List<sNatNetDiscoveredServer> m_discoveredServers = new();
        ...
        public NatNetServerDiscovery(IEnumerable<string> knownServerAddresses = null)
        {
            ...
            {
                var result =
                    NativeMethods.NatNet_CreateAsyncServerDiscovery(out m_discoveryHandle, m_nativeCallbackHandler);
            ...
            {
                var result = NativeMethods.NatNet_CreateAsyncServerDiscovery(out m_discoveryHandle,
                    m_nativeCallbackHandler, IntPtr.Zero, false);
                NatNetException.ThrowIfNotOK(result, "NatNet_CreateAsyncServerDiscovery failed.");

                foreach (var serverAddress in knownServerAddresses)
                {
                    result = NativeMethods.NatNet_AddDirectServerToAsyncDiscovery(m_discoveryHandle, serverAddress);
                    NatNetException.ThrowIfNotOK(result, "NatNet_AddDirectServerToAsyncDiscovery failed.");
                }

                result = NativeMethods.NatNet_StartAsyncDiscovery(m_discoveryHandle);
                NatNetException.ThrowIfNotOK(result, "NatNet_StartAsyncDiscovery failed.");
            }
        }


        private void ServerDiscoveredNativeThunk(sNatNetDiscoveredServer discoveredServer, IntPtr pUserContext)
        {
            try
            {
                ThrowIfDisposed();
...
#pragma warning restore CA1031 // Do not catch general exception types
            {
                ...
                // from this reverse P/Invoke delegate would transform into an SEH exception once
                // propagated across the native code boundary, and NatNetLib would blow up.
                System.Diagnostics.Debug.WriteLine("ERROR - Exception occurred in ServerDiscoveredNativeThunk: " +
                                                   ex.ToString());
            }
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            var destroyResult = NativeMethods.NatNet_FreeAsyncServerDiscovery(m_discoveryHandle);

            if (destroyResult != NatNetError.NatNetError_OK)
                System.Diagnostics.Debug.WriteLine("NatNet_FreeAsyncServerDiscovery returned " +
    ...
    internal class NatNetClient : IDisposable
    {
        ...
        public static Version NatNetLibVersion
        {
            get
            {
                var natNetLibVersion = new byte[4];
                NativeMethods.NatNet_GetVersion(natNetLibVersion);
        ...
        public sServerDescription ServerDescription => m_serverDesc;
            ...
            public sFrameOfMocapData MarshaledFrame
            {
                get
                {
                    if (m_marshaledFrame.HasValue == false)
                        m_marshaledFrame =
                            (sFrameOfMocapData)Marshal.PtrToStructure(NativeFramePointer, typeof(sFrameOfMocapData));
        ...
        private NativeFrameReceivedEventArgs m_nativeFrameReceivedEventArgs = new();
        ...
        public NatNetClient()
        {
            var retval = NativeMethods.NatNet_Client_Create(out m_clientHandle);
            NatNetException.ThrowIfNotOK(retval, "NatNet_Client_Create failed.");

            if (m_clientHandle == IntPtr.Zero) throw new NatNetException("NatNet_Client_Create returned null handle.");

            // This ensures the reverse P/Invoke delegate passed to the native code stays alive.
            m_nativeFrameReceivedHandler = FrameReceivedNativeThunk;

            retval = NativeMethods.NatNet_Client_SetFrameReceivedCallback(m_clientHandle, m_nativeFrameReceivedHandler);
        ...
        public void Connect(NatNetConnectionType connType,
            ...
            ushort serverCommandPort = NatNetConstants.DefaultCommandPort,
            ushort serverDataPort = NatNetConstants.DefaultDataPort,
            IPAddress multicastAddress = null)
        {
            ThrowIfDisposed();

            var initParams = new sNatNetClientConnectParams
            {
            ...
            result = NativeMethods.NatNet_Client_Connect(m_clientHandle, ref initParams);
            NatNetException.ThrowIfNotOK(result, "NatNet_Client_Connect failed.");

            result = NativeMethods.NatNet_Client_GetServerDescription(m_clientHandle, out m_serverDesc);
        ...
        public void Disconnect()
        {
            ...
            if (Connected)
            {
                var retval = NativeMethods.NatNet_Client_Disconnect(m_clientHandle);
        ...
        public NatNetError Request(string request, out IntPtr pResponse, out int pResponseLenBytes,
            int timeoutMs = 1000, int numAttempts = 1)
        {
            ThrowIfDisposed();

            var retval = NativeMethods.NatNet_Client_Request(m_clientHandle, request, out pResponse,
                out pResponseLenBytes, timeoutMs, numAttempts);
        ...
        public float RequestFloat(string request, int timeoutMs = 1000, int numAttempts = 1)
        {
            ThrowIfDisposed();

            IntPtr responsePtr;
            int responseLen;
            var result = NativeMethods.NatNet_Client_Request(m_clientHandle, request, out responsePtr, out responseLen,
                timeoutMs, numAttempts);
        ...
        public int RequestInt32(string request, int timeoutMs = 1000, int numAttempts = 1)
        {
            ThrowIfDisposed();
            ...
            if (responseLen != Marshal.SizeOf(typeof(int))) throw new NatNetException("Response has incorrect length");

            int[] responseArray = { int.MinValue };
            Marshal.Copy(responsePtr, responseArray, 0, 1);
            return responseArray[0];
        }


        public bool RequestCommand(string request, int timeoutMs = 1000, int numAttempts = 1)
        {
            ThrowIfDisposed();
        ...
        public DataDescriptions GetDataDescriptions(uint descriptionTypesMask = 0xFFFFFFFF)
        {
            ThrowIfDisposed();

            IntPtr pDataDescriptions;
            var retval =
                NativeMethods.NatNet_Client_GetDataDescriptionList(m_clientHandle, out pDataDescriptions,
                    descriptionTypesMask);
            NatNetException.ThrowIfNotOK(retval, "NatNet_Client_GetDataDescriptions failed.");

            var dataDescriptions =
                (sDataDescriptions)Marshal.PtrToStructure(pDataDescriptions, typeof(sDataDescriptions));

            // Do a quick first pass to determine the required capacity for the returned lists.
            var numMarkerSetDescs = 0;
            var numRigidBodyDescs = 0;
            var numSkeletonDescs = 0;
            var numForcePlateDescs = 0;
            var numCameraDescs = 0;

            for (var i = 0; i < dataDescriptions.DataDescriptionCount; ++i)
            {
                var desc = dataDescriptions.DataDescriptions[i];

                switch (desc.DescriptionType)
                {
                    case (int)NatNetDataDescriptionType.NatNetDataDescriptionType_MarkerSet:
                        ++numMarkerSetDescs;
                        break;
                    case (int)NatNetDataDescriptionType.NatNetDataDescriptionType_RigidBody:
                        ++numRigidBodyDescs;
                        break;
                    case (int)NatNetDataDescriptionType.NatNetDataDescriptionType_Skeleton:
                        ++numSkeletonDescs;
                        break;
                    case (int)NatNetDataDescriptionType.NatNetDataDescriptionType_ForcePlate:
                        ++numForcePlateDescs;
                        break;
                    case (int)NatNetDataDescriptionType.NatNetDataDescriptionType_Camera:
                        ++numCameraDescs;
                        break;
                }
            }
            ...
            var retDescriptions = new DataDescriptions
            {
            ...
            for (var i = 0; i < dataDescriptions.DataDescriptionCount; ++i)
            {
                ...
                switch (desc.DescriptionType)
                {
                    case (int)NatNetDataDescriptionType.NatNetDataDescriptionType_MarkerSet:
                        var markerSetDesc =
                            (sMarkerSetDescription)Marshal.PtrToStructure(desc.Description,
                                typeof(sMarkerSetDescription));
                        retDescriptions.MarkerSetDescriptions.Add(markerSetDesc);
                        break;
                    case (int)NatNetDataDescriptionType.NatNetDataDescriptionType_RigidBody:
                        var rigidBodyDesc =
                            (sRigidBodyDescription)Marshal.PtrToStructure(desc.Description,
                                typeof(sRigidBodyDescription));
                        retDescriptions.RigidBodyDescriptions.Add(rigidBodyDesc);
                        break;
                    case (int)NatNetDataDescriptionType.NatNetDataDescriptionType_Skeleton:
                        var skeletonDesc =
                            (sSkeletonDescription)Marshal.PtrToStructure(desc.Description,
                                typeof(sSkeletonDescription));
                        retDescriptions.SkeletonDescriptions.Add(skeletonDesc);
                        break;
                    case (int)NatNetDataDescriptionType.NatNetDataDescriptionType_ForcePlate:
                        var forcePlateDesc =
                            (sForcePlateDescription)Marshal.PtrToStructure(desc.Description,
                                typeof(sForcePlateDescription));
                        retDescriptions.ForcePlateDescriptions.Add(forcePlateDesc);
                        break;
                    case (int)NatNetDataDescriptionType.NatNetDataDescriptionType_Camera:
                        var cameraDesc =
                            (sCameraDescription)Marshal.PtrToStructure(desc.Description, typeof(sCameraDescription));
        ...
        public NatNetError GetPredictedRigidBodyPose(int rbId, out sRigidBodyData rbData, double dt)
        {
            return NativeMethods.NatNet_Client_GetPredictedRigidBodyPose(m_clientHandle, rbId, out rbData, dt);
        }
...
#pragma warning restore CA1031 // Do not catch general exception types
            {
                ...
                // from this reverse P/Invoke delegate would transform into an SEH exception once
                // propagated across the native code boundary, and NatNetLib would blow up.
                System.Diagnostics.Debug.WriteLine("ERROR - Exception occurred in FrameReceivedNativeThunk: " +
                                                   ex.ToString());
            }
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (Connected)
            {
                var disconnectResult = NativeMethods.NatNet_Client_Disconnect(m_clientHandle);

                if (disconnectResult != NatNetError.NatNetError_OK)
                    System.Diagnostics.Debug.WriteLine("NatNet_Client_Disconnect returned " +
                                                       disconnectResult.ToString() + ".");

                Connected = false;
            }

            // Now destroy the native client.
            var destroyResult = NativeMethods.NatNet_Client_Destroy(m_clientHandle);
