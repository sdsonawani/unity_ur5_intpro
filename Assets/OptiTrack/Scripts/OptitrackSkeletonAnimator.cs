...
public class OptitrackSkeletonAnimator : MonoBehaviour
{
    ...
    private HumanPose m_humanPose = new();
    ...
    private Dictionary<int, GameObject> m_boneObjectMap;
    ...
    /// </summary>
    private Dictionary<string, string> m_cachedMecanimBoneNameMap = new();
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError(
                GetType().FullName + ": Could not find skeleton definition with the name \"" + SkeletonAssetName + "\"",
                this);
            enabled = false;
            return;
        }
        ...
        var rootObjectName = "OptiTrack Skeleton - " + SkeletonAssetName;
        m_rootObject = new GameObject(rootObjectName);

        m_boneObjectMap = new Dictionary<int, GameObject>(m_skeletonDef.Bones.Count);

        for (var boneDefIdx = 0; boneDefIdx < m_skeletonDef.Bones.Count; ++boneDefIdx)
        {
            var boneDef = m_skeletonDef.Bones[boneDefIdx];

            var boneObject = new GameObject(boneDef.Name);
            boneObject.transform.parent = boneDef.ParentId == 0
                ? m_rootObject.transform
                : m_boneObjectMap[boneDef.ParentId].transform;
        ...
        m_rootObject.transform.parent = StreamingClient.transform;
    ...
    private void Update()
    {
        var skelState = StreamingClient.GetLatestSkeletonState(m_skeletonDef.Id);
        if (skelState != null)
        {
            // Update the transforms of the bone GameObjects.
            for (var i = 0; i < m_skeletonDef.Bones.Count; ++i)
            {
                ...
                var foundObject = m_boneObjectMap.TryGetValue(boneId, out boneObject);
    ...
    private void MecanimSetup(string rootObjectName)
    {
        ...
        var humanBones = new List<HumanBone>(m_skeletonDef.Bones.Count);
        for (var humanBoneNameIdx = 0; humanBoneNameIdx < humanTraitBoneNames.Length; ++humanBoneNameIdx)
        {
            var humanBoneName = humanTraitBoneNames[humanBoneNameIdx];
        ...
        var skeletonBones = new List<SkeletonBone>(m_skeletonDef.Bones.Count + 1);
        ...
        if (m_srcAvatar.isValid == false || m_srcAvatar.isHuman == false)
        {
            Debug.LogError(
                GetType().FullName +
                ": Unable to create source Avatar for retargeting. Check that your Skeleton Asset Name and Bone Naming Convention are configured correctly.",
                this);
            enabled = false;
            return;
        }
        ...
        m_destPoseHandler = new HumanPoseHandler(DestinationAvatar, transform);
    }
