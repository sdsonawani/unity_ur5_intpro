...
public class SubscribeUr5Joints : MonoBehaviour
{
    ...
    {
        "world/dummy_link/base_link/shoulder_link",
        "/upper_arm_link",
        "/forearm_link",
        "/wrist_1_link",
        "/wrist_2_link",
    ...
    public static readonly string[] EndEffectorLinkNames_Right =
    {
    ...
    public static readonly string[] EndEffectorLinkNames_Left =
    {
    ...
    private ArticulationBody[] m_ur5ArticulationBody;

    private ArticulationBody[] m_robotiqGripperR;
    private ArticulationBody[] m_robotiqGripperL;
    ...
    private void jointCallback(Ur5JointsMsg JointMsg)
    {
        for (var i = 0; i < num_joints; i++) _joints[i] = JointMsg.joints[i];
    }

    private void robotiqCallback(Float64Msg gripperMsg)
    {
        GripperGoal = (float)gripperMsg.data;
        // GripperGoal = (float)10;
    }

    private void applyControlRobotiq(ArticulationBody[] RobotiqBody, string ee_type)
    {
        for (var i = 0; i < ee_joints; i++)
        {
            var eeDrive = RobotiqBody[i].xDrive;
            var gripper_goal = GripperGoal;
            ...
            if (i == 1)
            {
                ...
                if (ee_type == "left")
                {
                    ...
                    else if (p_istate_eel[i] < m_i_min) p_istate_eel[i] = m_i_min;
                    var velocity = p_gain_eel * error + p_istate_eel[i] * i_gain_eel +
                                   (error - p_dstate_eel[i]) * d_gain_eel;
                ...
                else if (ee_type == "right")
                {
                    ...
                    else if (p_istate_eer[i] < m_i_min) p_istate_eer[i] = m_i_min;
                    var velocity = p_gain_eer * error + p_istate_eer[i] * i_gain_eer +
                                   (error - p_dstate_eer[i]) * d_gain_eer;
                    p_dstate_eel[i] = (float)error;
                    eeDrive.damping = 1e+18f;
                    eeDrive.stiffness = 1e+18f;
                    eeDrive.forceLimit = 1e+20f;
    ...
    private void applyControlUR5(ArticulationBody[] ur5_body)
    {
        // init debug values for current and goal joints
        var joints_c_debug = new float[6];
        var joints_g_debug = new float[6];
        ...
        for (var i = 0; i < num_joints; i++)
        {
            ...
            if (p_istate[i] > m_i_max)
                p_istate[i] = m_i_max;
            else if (p_istate[i] < m_i_min) p_istate[i] = m_i_min;
